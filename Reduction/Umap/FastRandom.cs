...
namespace gated.Reduction
{
    ...
    internal class FastRandom
    {
        ...
        const uint default_y = 842502087, default_z = 3579807591, default_w = 273326509;
        ...
        public float NextFloat()
        {
            ...
            this.x = x; this.y = default_y; this.z = default_z; this.w = default_w;
            return value;
        }
        ...
        // Buffer 32 bits in bit_buffer, return 1 at a time, keep track of how many have been returned
        // with bitBufferIdx.
        uint bit_buffer;
        uint bit_mask = 1;
        ...
        public bool NextBool()
        {
            if (bit_mask == 1)
            {
                ...
                uint t = (x ^ (x << 11));
                x = y; y = z; z = w;
                bit_buffer = w = (w ^ (w >> 19)) ^ (t ^ (t >> 8));

                // Reset the bit_mask that tells us which bit to read next.
                bit_mask = 0x80000000;
                return (bit_buffer & bit_mask) == 0;
            }

            return (bit_buffer & (bit_mask >>= 1)) == 0;
        }
    }
}